<template>
  <div class="form-page">
    <div class="form-header">
      <button class="btn-back" @click="$router.push('/categorias')">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M19 12H5M12 5l-7 7 7 7"/></svg>
        Categorias
      </button>
      <h2 class="page-title">{{ pk ? 'Editar Categoria' : 'Nova Categoria' }}</h2>
    </div>
    ...
    <form v-else @submit.prevent="salvar" class="form-card">
      <div class="form-grid">
        ...
        <div class="field full"><label>Descrição</label><input v-model="form.descricao" type="text" /></div>
        <div class="field full check-field">
          <label class="check-label">
            <input type="checkbox" v-model="form.desconto_somente_decorador" />
            Desconto somente para decorador (10% automático no PDV)
          </label>
        </div>
      </div>
      ...
      <div class="form-actions">
        <button type="button" class="btn-cancel" @click="$router.push('/categorias')">Cancelar</button>
...
const form = ref({ nome: '', descricao: '', desconto_somente_decorador: false });

onMounted(async () => {
  if (!pk) return;
  const { data, error } = await supabase.from('categorias').select('*').eq('pk', pk).single();
  if (error || !data) { erro.value = 'Categoria não encontrada.'; carregando.value = false; return; }
  form.value = { ...data };
...
async function salvar() {
  ...
  try {
    const payload = {
      ...
      desconto_somente_decorador: form.value.desconto_somente_decorador || false,
      filial_pk:                 sessaoStore.filial?.pk || null,
    };
    let error;
    if (pk) {
      ({ error } = await supabase.from('categorias').update(payload).eq('pk', pk));
    } else {
...
<style scoped>
.form-page   { display: flex; flex-direction: column; gap: 1.25rem; max-width: 560px; }
